...
const CardPayment = ({ method, card, setCard, total }) => {
     ...
     {card && !card.id &&
      <form className='anime-left mt-3'>
       ...
       <div className="row mt-4">
        <div className={` ${method === 'debit_card' ? 'col-sm-12' : 'col-sm-8'}`}>
         <div className="form-floating">
         ...
         <div className="col-sm-4">
          <div className="form-floating">
           <select className="form-control text-center" id="name" type="text" value={card.installments.value}
            onChange={({ target }) => setCard({ ...card, installments: { ...card.installments, value: target.value, error: false } })}
