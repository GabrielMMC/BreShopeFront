...
const AddProduct = ({ edit }) => {
  const [state, setState] = React.useState({
    ...
    description3: { label: 'Descrição', value: "", error: false, col: 'col-sm-12', type: 'multiline', },
  ...
  function renderInput() {
    ...
    keys = keys.filter(item => item === 'name' || item === 'material' || item === 'description' || item === 'damage' || item === 'price')
    return (
      keys.map(item => (
